...
print_pdf = function(input, chrome = TRUE) {
  ...
  command = paste0(ifelse(chrome, "google-chrome", "chromium"), " --headless --print-to-pdf='pdf/mlr3.pdf' --virtual-time-budget=20000  --disable-gpu --run-all-compositor-stages-before-draw ", svr$url)
  print(command)
